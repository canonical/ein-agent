"""Human-in-the-loop orchestrator for interactive investigation workflows."""
...
class HITLOrchestrator:
    ...
    async def _handle_approval_interruptions(
        ...
        for idx, interruption in enumerate(interruptions, 1):
            ...
            console.print_message(
                '  [dim]y[/dim] - Approve once      [dim]a[/dim] - Approve always'
            )
            console.print_message(
                '  [dim]n[/dim] - Reject once       [dim]r[/dim] - Reject always'
            )
            print()
            ...
            try:
                ...
                while True:
                    ...
                    if response == '[HANDOFF]':
                        ...
                        if handoff:
                            msg = (
                                f"Agent '{handoff['from']}' wants to hand off"
                                f" to '{handoff['to']}'."
                            )
